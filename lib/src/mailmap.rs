...
pub async fn read_current_mailmap(
    ...
    let path = RepoPath::from_internal_string(".mailmap").unwrap();
    ...
    let MaterializedTreeValue::File(MaterializedFileValue { mut reader, id, .. }) =
        materialize_tree_value(repo.store(), path, value, tree.labels()).await?
    else {
        return Ok(Mailmap::empty());
    };
