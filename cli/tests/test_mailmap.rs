...
fn test_mailmap() {
    ...
    let repo_env = test_env.work_dir("repo");

    let mut mailmap = String::new();
    let mut append_mailmap = |extra| {
        mailmap.push_str(extra);
        repo_env.write_file(".mailmap", &mailmap);
    };

    let run_as = |name: &str, email: &str, args: &[&str]| {
        repo_env
            .run_jj_with(|cmd| cmd.args(args).env("JJ_USER", name).env("JJ_EMAIL", email))
            .success()
    };

    append_mailmap("# test comment\n");

    let stdout = repo_env.run_jj(&["log", "-T", "author"]).success();
    ...
    let stdout = repo_env
        .run_jj(&["log", "-T", "author", "--at-operation=@-"])
        .success();
    insta::assert_snapshot!(stdout, @r#"
    @  Vest User <user@test>
    ○  Rest User <user@test>
    ...
    let stdout = repo_env
        .run_jj(&["log", "-T", "author", "-r", "author(substring-i:bEsT)"])
        .success();
    insta::assert_snapshot!(stdout, @r#"
    ○  Best User <best.user@example.com>
    │
    ...
    let stdout = repo_env
        .run_jj(&["log", "-T", "author", "-r", "author(pest)"])
        .success();
    insta::assert_snapshot!(stdout, @r###"
    "###);
    ...
    // commit data.
    let stdout = repo_env
        .run_jj(&["log", "-T", "author", "-r", "author_raw(\"user@test\")"])
        .success();
    insta::assert_snapshot!(stdout, @r#"
    @  Zest User <zest.user@example.org>
    ○  Jest User <jest.user@example.org>
